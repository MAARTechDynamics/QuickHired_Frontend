...
@Component({
  selector: 'app-multi-step-interview',
  standalone: true,
  imports: [HttpClientModule, RouterModule, CommonModule, ReactiveFormsModule, FormsModule, TranslatePipe],
  templateUrl: './multi-step-interview.component.html',
  styleUrl: './multi-step-interview.component.css'
})
export class MultiStepInterviewComponent implements OnInit {
  ...
  durations = [
    ...
    { id: '45m', name: '45m', label: 'Extended' },
  ...
  loadUserSubscriptionDetails() {
    ...
    this.http.get<any>(`${environment.apiBaseUrl}/api/Membership/user-plan/${this.userId}`).subscribe({
      next: (res) => {
        this.planSessionLimit = res.totalSessions;    // e.g., 3
  ...
  checkUserLiveSessionCount() {
    ...
    this.http.get<any[]>(`${environment.apiBaseUrl}/api/Membership/user-sessions/${this.userId}`).subscribe({
      next: (res) => {
        this.userLiveSessionsCount = res.filter(x => x.interviewType === 'Live').length;
  ...
  quickStartInterview() {
    ...
    if (this.personaForm.invalid) {
      ...
      Swal.fire({
        ...
        text: 'Please complete all required fields: resume, company name, job title, job description, interview domain, and session duration.'
      });
      return;
    }
  ...
  // UI helper method for mini-summary display
  getDomainName(domainId: string): string {
    const domain = this.domains.find(d => d.id === domainId);
    return domain ? domain.name : 'Not selected';
  }
}
